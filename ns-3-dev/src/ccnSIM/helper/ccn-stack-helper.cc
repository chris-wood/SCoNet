...
#include "../model/ccn-net-device-face.h"
#include "../model/ccn-l3-protocol.h"
...
#include "ns3/ccn-security-module.h"
...
#include "ccn-face-container.h"
#include "ccn-stack-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("ccn.StackHelper");
...
{
  m_ccnFactory.         SetTypeId ("ns3::ccn::L3Protocol");
  m_strategyFactory.    SetTypeId ("ns3::ccn::fw::Flooding");
  //m_contentStoreFactory.SetTypeId ("ns3::ccn::ContentStore");
  m_contentStoreFactory.SetTypeId ("ns3::ccn::cs::UtilityCaching");
  m_fibFactory.         SetTypeId ("ns3::ccn::fib::Default");
  m_pitFactory.         SetTypeId ("ns3::ccn::pit::Persistent");
...
{
  if (attr1 != "")
      m_ccnFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
      m_ccnFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
      m_ccnFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
      m_ccnFactory.Set (attr4, StringValue (value4));
}
...
void
StackHelper::SetSecurityModule (const std::string &securityModule,
                              const std::string &attr1, const std::string &value1,
                              const std::string &attr2, const std::string &value2,
                              const std::string &attr3, const std::string &value3,
                              const std::string &attr4, const std::string &value4)
{
  m_securityModuleFactory.SetTypeId (securityModule);
  if (attr1 != "")
      m_securityModuleFactory.Set (attr1, StringValue (value1));
  if (attr2 != "")
      m_securityModuleFactory.Set (attr2, StringValue (value2));
  if (attr3 != "")
      m_securityModuleFactory.Set (attr3, StringValue (value3));
  if (attr4 != "")
      m_securityModuleFactory.Set (attr4, StringValue (value4));
}
void
  ...
  if (node->GetObject<L3Protocol> () != 0)
    {
      NS_FATAL_ERROR ("StackHelper::Install (): Installing "
                      "a CcnStack to a node with an existing Ccn object");
      return 0;
    }

  // Create L3Protocol
  Ptr<L3Protocol> ccn = m_ccnFactory.Create<L3Protocol> ();

  // Create and aggregate FIB
  Ptr<Fib> fib = m_fibFactory.Create<Fib> ();
  ccn->AggregateObject (fib);

  // Create and aggregate PIT
  ccn->AggregateObject (m_pitFactory.Create<Pit> ());

  // Create and aggregate forwarding strategy
  ccn->AggregateObject (m_strategyFactory.Create<ForwardingStrategy> ());

  // Create and aggregate content store
  ccn->AggregateObject (m_contentStoreFactory.Create<ContentStore> ());

  // Aggregate L3Protocol on node
  node->AggregateObject (ccn);

  for (uint32_t index=0; index < node->GetNDevices (); index++)
    {
      Ptr<NetDevice> device = node->GetDevice (index);
      // This check does not make sense: LoopbackNetDevice is installed only if IP stack is installed,
      // Normally, ccnSIM works without IP stack, so no reason to check
      ...
      for (std::list< std::pair<TypeId, NetDeviceFaceCreateCallback> >::const_iterator item = m_netDeviceCallbacks.begin ();
           ...
           item++)
        {
          if (device->GetInstanceTypeId () == item->first ||
            ...
            {
              face = item->second (node, ccn, device);
              if (face != 0)
                break;
            }
        }
      if (face == 0)
        {
          face = DefaultNetDeviceCallback (node, ccn, device);
        }

      if (m_needSetDefaultRoutes)
        {
          // default route with lowest priority possible
          AddRoute (node, "/", StaticCast<Face> (face), std::numeric_limits<int32_t>::max (), std::numeric_limits<int32_t>::max ());
        }
...
Ptr<NetDeviceFace>
StackHelper::DefaultNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ccn, Ptr<NetDevice> netDevice) const
{
...
Ptr<NetDeviceFace>
StackHelper::PointToPointNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ccn, Ptr<NetDevice> device) const
{
...
void
StackHelper::AddRoute (Ptr<Node> node, const std::string &prefix, Ptr<Face> face, int32_t metric, int32_t rank)
{
  ...
  fib->Add (prefixValue.Get (), face, metric, rank);
}

void
StackHelper::AddRoute (Ptr<Node> node, const std::string &prefix, uint32_t faceId, int32_t metric, int32_t rank)
{
  Ptr<L3Protocol>     ccn = node->GetObject<L3Protocol> ();
  NS_ASSERT_MSG (ccn != 0, "Ccn stack should be installed on the node");

  Ptr<Face> face = ccn->GetFace (faceId);
  NS_ASSERT_MSG (face != 0, "Face with ID [" << faceId << "] does not exist on node [" << node->GetId () << "]");

  AddRoute (node, prefix, face, metric, rank);
}

void
StackHelper::AddRoute (const std::string &nodeName, const std::string &prefix, uint32_t faceId, int32_t metric, int32_t rank)
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
...
void
StackHelper::AddRoute (Ptr<Node> node, const std::string &prefix, Ptr<Node> otherNode, int32_t metric, int32_t rank)
{
  for (uint32_t deviceId = 0; deviceId < node->GetNDevices (); deviceId ++)
    {
      ...
      if (channel->GetDevice (0)->GetNode () == otherNode ||
          channel->GetDevice (1)->GetNode () == otherNode)
        {
          ...
          Ptr<Face> face = ccn->GetFaceByNetDevice (netDevice);
...
void
StackHelper::AddRoute (const std::string &nodeName, const std::string &prefix, const std::string &otherNodeName, int32_t metric, int32_t rank)
{
  Ptr<Node> node = Names::Find<Node> (nodeName);
  NS_ASSERT_MSG (node != 0, "Node [" << nodeName << "] does not exist");
  ...
  AddRoute (node, prefix, otherNode, metric, rank);
}
