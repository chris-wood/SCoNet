...
#ifndef CCN_STACK_HELPER_H
#define CCN_STACK_HELPER_H
...
/**
 * \ingroup ccn
 * \defgroup ccn-helpers Helpers
 */
/**
 * \ingroup ccn-helpers
 * \brief Adding Ccn functionality to existing Nodes.
 *
 * This helper enables pcap and ascii tracing of events in the ccn stack
 ...
 * PcapUserHelperForCcn and AsciiTraceUserHelperForCcn are treated as
...
public:
  /**
   * \brief Create a new CcnStackHelper with a default CCN_FLOODING forwarding stategy
   */
  StackHelper();

  /**
   * \brief Destroy the CcnStackHelper
   */
  virtual ~StackHelper ();

  /**
   * @brief Set parameters of CcnL3Protocol
   */
  void
   ...
   *
   * Valid options are "ns3::CcnFloodingStrategy" (default) and "ns3::CcnBestRouteStrategy"
   *
   * Other strategies can be implemented, inheriting ns3::CcnForwardingStrategy class
   */
  void
   ...
   */
  void
  SetSecurityModule (const std::string &securityModuleClass,
                   const std::string &attr1 = "", const std::string &value1 = "",
                   const std::string &attr2 = "", const std::string &value2 = "",
                   const std::string &attr3 = "", const std::string &value3 = "",
                   const std::string &attr4 = "", const std::string &value4 = "");
  ...
  /**
   * \brief Install Ccn stack on the node
   *
   * This method will assert if called on a node that already has Ccn object
   * installed on it
   *
   * \param nodeName The name of the node on which to install the stack.
   *
   * \returns list of installed faces in the form of a smart pointer
   * to CcnFaceContainer object
   */
  Ptr<FaceContainer>
  ...
  /**
   * \brief Install Ccn stack on each node in the input container
   *
   * The program will assert if this method is called on a container with a node
   * that already has an ccn object aggregated to it.
   *
  ...
  /**
   * \brief Install Ccn stack on all nodes in the simulation
   *
   * \returns list of installed faces in the form of a smart pointer
   * to FaceContainer object
   */
   ...
   */
  static void
  AddRoute (const std::string &nodeName, const std::string &prefix, uint32_t faceId, int32_t metric, int32_t rank);
   ...
   */
  static void
  AddRoute (Ptr<Node> node, const std::string &prefix, uint32_t faceId, int32_t metric, int32_t rank);
   ...
   */
  static void
  AddRoute (Ptr<Node> node, const std::string &prefix, Ptr<Face> face, int32_t metric, int32_t rank);
   ...
   */
  static void
  AddRoute (Ptr<Node> node, const std::string &prefix, Ptr<Node> otherNode, int32_t metric, int32_t rank);
   ...
   */
  static void
  AddRoute (const std::string &nodeName, const std::string &prefix, const std::string &otherNodeName, int32_t metric, int32_t rank);
...
private:
  Ptr<NetDeviceFace>
  DefaultNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ccn, Ptr<NetDevice> netDevice) const;

  Ptr<NetDeviceFace>
  PointToPointNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ccn, Ptr<NetDevice> netDevice) const;
...
private:
  ObjectFactory m_ccnFactory;
  ObjectFactory m_strategyFactory;
  ObjectFactory m_contentStoreFactory;
  ObjectFactory m_securityModuleFactory;
