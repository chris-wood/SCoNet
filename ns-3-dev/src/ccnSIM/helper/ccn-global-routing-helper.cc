 ...
 *
 *
 * Modified to support CCNX
 * Maziar Mirzazad-Barijough <maziar@soe.ucsc.edu>
...
#include "ccn-global-routing-helper.h"
...
#include "boost-graph-ccn-global-routing-helper.h"

#include <math.h>

NS_LOG_COMPONENT_DEFINE ("ccn.GlobalRoutingHelper");
...
GlobalRoutingHelper::Install (Ptr<Node> node)
{
  ...
  NS_ASSERT_MSG (ccn != 0, "Cannot install GlobalRoutingHelper before Ccn is installed on a node");
  ...
  for (uint32_t faceId = 0; faceId < ccn->GetNFaces (); faceId++)
    {
      Ptr<NetDeviceFace> face = DynamicCast<NetDeviceFace> (ccn->GetFace (faceId));
      if (face == 0)
	{
...
GlobalRoutingHelper::CalculateRoutes (bool invalidatedRoutes/* = true*/)
{
  ...
  BOOST_CONCEPT_ASSERT(( VertexListGraphConcept< CcnGlobalRouterGraph > ));
  BOOST_CONCEPT_ASSERT(( IncidenceGraphConcept< CcnGlobalRouterGraph > ));

  CcnGlobalRouterGraph graph;
  // typedef graph_traits < CcnGlobalRouterGraph >::vertex_descriptor vertex_descriptor;
                  ...
                  BOOST_FOREACH (const Ptr<const Name> &prefix, i->first->GetLocalPrefixes ())
                    {
                      ...
                      Ptr<fib::Entry> entry = fib->Add (prefix, i->second.get<0> (), i->second.get<1> (),i->second.get<1> ());
  ...
  for (NodeList::Iterator node = NodeList::Begin (); node != NodeList::End (); node++)
      ...
      if (source == 0)
	{
      ...
      for (uint32_t enabledFaceId = 0; enabledFaceId < l3->GetNFaces (); enabledFaceId++)
        {
          if (DynamicCast<ccn::NetDeviceFace> (l3->GetFace (enabledFaceId)) == 0)
            continue;
