...
#ifndef _CCN_TLV_PAYLOAD_H_
#define _CCN_TLV_PAYLOAD_H_
/* CCN 1.0 TLV DATA PAYLOAD HDR packet
                    1                   2                   3
 ...
 **/
    class TLV_PAYLOAD:public TLV_RAWDATA
    {
    public:
   ...
   **/
      TLV_PAYLOAD (const TLV_PAYLOAD & tlv_msg);

  /**
   * @brief Copy constructor from serialized byte data
   **/
        TLV_PAYLOAD (const TLV_RAW_DATA_t * rawData);

    /**
   * @brief Copy constructor from XML formatted string 
   **/
        TLV_PAYLOAD (const std::string & xmlString);

 /**
    * @brief Copy constructor from type, length and raw data information
    **/
        TLV_PAYLOAD (TLV_TYPE_t type, TLV_LENGTH_t length,
...
/**
 ...
 tlvIdName() const;
  ...
  /**
    ...
    *
    **/
      virtual TLV_PAYLOAD & operator = (const TLV_PAYLOAD & other);
