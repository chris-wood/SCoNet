...
namespace wire
{
  namespace icnx
  {
    ...
    TLV_CERT::TLV_CERT (const TLV_CERT & tlv_msg)
      :TLV_RAWDATA(tlv_msg)
    {
      //m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
    ...
    TLV_CERT::TLV_CERT (const std::string & xmlString)
      :TLV_RAWDATA(xmlString)
    {
     // m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
     // m_type = MSG_NAME_CERT_TLV;
      //XmlImplement (xmlString);
    }

    TLV_CERT::TLV_CERT (const TLV_RAW_DATA_t * rawData)
      :TLV_RAWDATA(rawData)
    {
      //m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
    ...
    TLV_CERT::TLV_CERT (TLV_TYPE_t type, TLV_LENGTH_t length,
 ...
 const std::string 
 TLV_CERT::tlvIdName() const { return std::string("TLV_CERT"); }


    TLV_CERT & TLV_CERT::operator = (const TLV_CERT & other)
    {
...
main ()
{
  ...
  wire::icnx::TLV_CERT a;
  const uint8_t *var =
    reinterpret_cast < const uint8_t * >("ABCDEFGHIJKLMNOPQRSTUVXYZ");
  wire::icnx::TLV_CERT b (257, 26, var); //add 1 for NULL for strings
  wire::icnx::TLV_CERT c (b);

  wire::icnx::TLV_CERT d (b.NetworkSerialize ());
  wire::icnx::TLV_CERT e (b.XmlPrint ());

  wire::icnx::TLV_CERT f;
  f.SetData ("0123456789");
  wire::icnx::TLV_CERT *g = new wire::icnx::TLV_CERT;
  *g = f;
