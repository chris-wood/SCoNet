...
namespace wire
{
  namespace icnx
  {
    ...
    TLV_LINK::TLV_LINK (const TLV_LINK & tlv_msg)
      :TLV_RAWDATA(tlv_msg)
    {
      //m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
    ...
    TLV_LINK::TLV_LINK (const std::string & xmlString)
      :TLV_RAWDATA(xmlString)
    {
     // m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
     // m_type = MSG_NAME_LINK_TLV;
      //XmlImplement (xmlString);
    }

    TLV_LINK::TLV_LINK (const TLV_RAW_DATA_t * rawData)
      :TLV_RAWDATA(rawData)
    {
      //m_serializedDataPtr = NULL;
     // m_DataPtr = NULL;
    ...
    TLV_LINK::TLV_LINK (TLV_TYPE_t type, TLV_LENGTH_t length,
 ...
 const std::string 
 TLV_LINK::tlvIdName() const { return std::string("TLV_LINK"); }

    TLV_BASE * 
    TLV_LINK::Copy() const
    {
         return new TLV_LINK( *this); 
    }

    TLV_LINK & TLV_LINK::operator = (const TLV_LINK & other)
    {
...
main ()
{
  ...
  wire::icnx::TLV_LINK a;
  const uint8_t *var =
    reinterpret_cast < const uint8_t * >("ABCDEFGHIJKLMNOPQRSTUVXYZ");
  wire::icnx::TLV_LINK b (257, 26, var); //add 1 for NULL for strings
  wire::icnx::TLV_LINK c (b);

  wire::icnx::TLV_LINK d (b.NetworkSerialize ());
  wire::icnx::TLV_LINK e (b.XmlPrint ());

  wire::icnx::TLV_LINK f;
  f.SetData ("0123456789");
  wire::icnx::TLV_LINK *g = new wire::icnx::TLV_LINK;
  *g = f;
