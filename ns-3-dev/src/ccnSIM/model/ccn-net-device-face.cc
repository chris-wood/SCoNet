...
#include "ccn-net-device-face.h"
#include "ccn-l3-protocol.h"
...
NS_LOG_COMPONENT_DEFINE ("ccn.NetDeviceFace");
...
{
  static TypeId tid = TypeId ("ns3::ccn::NetDeviceFace")
    .SetParent<Face> ()
    .SetGroupName ("Ccn")
    ;
  return tid;
...
/**
 * By default, Ccn face are created in the "down" state.  Before
...
void
NetDeviceFace::RegisterProtocolHandlers (const InterestHandler &interestHandler, const DataHandler &dataHandler, const InterestResponseHandler &interestResponseHandler)
{
  NS_LOG_FUNCTION (this);

  Face::RegisterProtocolHandlers (interestHandler, dataHandler, interestResponseHandler);
...
//std::cout << "net send packet is size:" << packet->GetSize() << std::endl;
  if (!Face::Send (packet))
    {
      return false;
    }
  ...
  NS_ASSERT_MSG (packet->GetSize () <= m_netDevice->GetMtu (),
                 ...
                 << m_netDevice->GetMtu ()
                 << " for Ccn; fragmentation not supported");
