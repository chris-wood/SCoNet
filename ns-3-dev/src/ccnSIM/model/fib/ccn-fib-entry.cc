...
#define CCN_RTO_ALPHA 0.125
#define CCN_RTO_BETA 0.25
#define CCN_RTO_K 4
...
NS_LOG_COMPONENT_DEFINE ("ccn.fib.Entry");
...
FaceMetric::UpdateRtt (const Time &rttSample)
{
  ...
  if (m_sRtt.IsZero ())
    ...
    {
      m_rttVar = Time ((1 - CCN_RTO_BETA) * m_rttVar + 1.0 * CCN_RTO_BETA * Abs(m_sRtt - rttSample));
      m_sRtt = Time ((1 - CCN_RTO_ALPHA) * m_sRtt + 1.0 * CCN_RTO_ALPHA * rttSample);
    }
}
...
void
Entry::AddOrUpdateRoutingMetric (Ptr<Face> face, int32_t metric, int32_t rank)
{
  ...
  if (record == m_faces.get<i_face> ().end ())
    {
      m_faces.insert (FaceMetric (face, metric, rank));
    }
  else
  {
    // don't update metric to higher value
    if (record->GetRoutingCost () > metric || record->GetStatus () == FaceMetric::CCN_FIB_RED)
      {
        m_faces.modify (record,
                        ll::bind (&FaceMetric::SetRoutingCost, ll::_1, metric));

        m_faces.modify (record,
                        ll::bind (&FaceMetric::SetStatus, ll::_1, FaceMetric::CCN_FIB_YELLOW));
      }
  }
...
Entry::Invalidate ()
{
  for (FaceMetricByFace::type::iterator face = m_faces.begin ();
       ...
       face++)
    {
      ...
      m_faces.modify (face,
                      ll::bind (&FaceMetric::SetStatus, ll::_1, FaceMetric::CCN_FIB_RED));
    }
}
