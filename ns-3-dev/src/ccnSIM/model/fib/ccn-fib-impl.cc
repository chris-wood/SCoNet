...
NS_LOG_COMPONENT_DEFINE ("ccn.fib.FibImpl");
...
{
  static TypeId tid = TypeId ("ns3::ccn::fib::Default") // cheating ns3 object system
    .SetParent<Fib> ()
    .SetGroupName ("Ccn")
...
Ptr<Entry>
FibImpl::Add (const Name &prefix, Ptr<Face> face, int32_t metric, int32_t rank)
{
  return Add (Create<Name> (prefix), face, metric, rank);
}
  
Ptr<Entry>
FibImpl::Add (const Ptr<const Name> &prefix, Ptr<Face> face, int32_t metric, int32_t rank)
{
  ...
  if (result.first != super::end ())
    {
      ...
      super::modify (result.first,
                     ll::bind (&Entry::AddOrUpdateRoutingMetric, ll::_1, face, metric, rank));
