...
#include <stdint.h>
#include "ccn-forwarding-strategy.h"
...
{
  static TypeId tid = TypeId ("ns3::ccn::ForwardingStrategy")
    .SetGroupName ("Ccn")
    .SetParent<Object> ()
...
ForwardingStrategy::NotifyNewAggregate ()
{
  ...
  if (m_securityModule == 0)
  {
    m_securityModule = GetObject<SecurityModule> ();
  }
...
bool
ForwardingStrategy::CheckCS(Ptr<Face> inFace, Ptr<Interest> interest){
	...
	if(contentObject!=0){
		...
		bool ok = inFace->SendData (contentObject);
			  ...
			  DidSendOutData (inFace, inFace, contentObject);
			  NS_LOG_DEBUG ("Satisfy " << *inFace);
...
void
ForwardingStrategy::OnInterestResponse (Ptr<Face> inFace,
                                ...
                                Ptr<Interest> interest)
{
  ...
  if (DynamicCast<NetDeviceFace> (inFace) != 0){
                            ...
                            Ptr<Data> data)
{
  ...
  if (pitEntry == 0)
    ...
    {
	  if(!OffPathInjection(inFace,data,pitEntry)) {
		  m_dropData (data, inFace);
		  return;
	  }
...
bool
ForwardingStrategy::OffPathInjection(Ptr<Face> inFace,
                            Ptr<Data> data, Ptr<pit::Entry> pitEntry)
{

	  BOOST_FOREACH (const fib::FaceMetric &metricFace, pitEntry->GetFibEntry ()->m_faces.get<fib::i_metric> ())
	    {
		  if(inFace==metricFace.GetFace ()) return true;
	    }
	  return false;
}
                                            ...
                                            Ptr<pit::Entry> pitEntry)
{
	if (inFace != 0)
	  ...
	  BOOST_FOREACH (const pit::IncomingFace &incoming, pitEntry->GetIncoming ())
	    {
	      bool ok = incoming.m_face->SendInterestResponse(response);

	      //DidSendOutData (inFace, incoming.m_face, data, pitEntry);
	      NS_LOG_DEBUG ("Response " << *incoming.m_face);

	      if (!ok)
	        {
	         // m_dropData (data, incoming.m_face);
	          NS_LOG_DEBUG ("Cannot response  to " << *incoming.m_face);
	        }
	    }
                                       ...
                                       Ptr<pit::Entry> pitEntry)
{
  ...
  if (!propagated && interest->GetInterestHopCount() == 0 ){
	  //return InterestResponse, cause: HopLimit == 0;
  }
}
                                        ...
                                        Ptr<pit::Entry> pitEntry)
{
  ...
  if ((interest->GetInterestHopCount() == 0) and (DynamicCast<AppFace> (outFace) == 0)){
	  //can not send the interest with zero hopLimit out of the node
	  return false;
  }
