...
#ifndef CCN_FORWARDING_STRATEGY_H
#define CCN_FORWARDING_STRATEGY_H
...
/**
 * @ingroup ccn
 * @defgroup ccn-fw CCN forwarding strategies
 */
...
/**
 * @ingroup ccn-fw
 * @brief Abstract base class for Ccn forwarding strategies
 */
...
public:
  ...
  /**
   * \brief Actual processing of incoming Ccn interests. Note, interests do not have payload
   *
  ...
  virtual void
  OnInterestResponse (Ptr<Face> face,
              Ptr<InterestResponse> interestResponse);
  /**
   * \brief Actual processing of incoming Ccn content objects
   *
  ...
  /**
     * \brief Checking for off path injection
     *
     * Check for Off Path Injection of data
     * @param face    incoming face
     * @param data    Data packet
     * @param pitEntry Pit Entry
     */
    virtual bool
    OffPathInjection (Ptr<Face> face,
            Ptr<Data> data, Ptr<pit::Entry> pitEntry);

    /**
       * \brief Check if Content Store currently has a copy of requested data
       *
       * Check if Content Store currently has a copy of requested data
       * @param face    incoming face
       * @param interest incoming interest
       */
    virtual bool
    CheckCS(Ptr<Face> inFace, Ptr<Interest> interest);
  /**
  ...
  /**
   * @brief Event fired every time face is added to CCN stack
  ...
  /**
   * @brief Event fired every time face is removed from CCN stack
   * @param face face to be removed
   *
   * For example, when an application terminates, AppFace is removed and this method called by CCN stack.
   */
  virtual void
...
protected:
   ...
   *
   * Note that if CCN node is receiving a similar interest (interest for the same name),
   ...
   *
   * @param inFace  incoming face
   * @param response    Interest Response
   * @param pitEntry an existing PIT entry, corresponding to the duplicated Interest
   */
  virtual void
  SatisfyPendingInterest (Ptr<Face> inFace, // 0 allowed (from cache)
                          Ptr<InterestResponse> response,
                          Ptr<pit::Entry> pitEntry);
  /**
...
protected:
  ...
  Ptr<SecurityModule> m_securityModule; ///< \brief Security module for evaluation 
...
#endif /* CCN_FORWARDING_STRATEGY_H */
