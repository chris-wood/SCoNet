...
class Name (object):
    ...
    def __init__ (self, 
                  ...
                  name = None):
        if name is not None:
            if isinstance (name, ns.ccnSIM.ccn.Name):
                self._name = name
            ...
            if value is not None:
                if isinstance (value, Name):
                    self._name = ns.ccnSIM.ccn.Name (value._name)
                else:
                    self._name = ns.ccnSIM.ccn.Name (value)
            else:
                self._name = ns.ccnSIM.ccn.Name ()

    @staticmethod
    def fromWire (wire):
        return Name (name = ns.ccnSIM.ccn.Wire.ToName (wire))

    def toWire (self):
        return ns.ccnSIM.ccn.Wire.FromName (self._name)
    ...
    def __repr__ (self):
        return "ccn.Name('" + self._name.toUri () + "')"
