...
class Data (object):
    ...
    def __init__(self,
        ...
        if data:
            ...
            elif isinstance (data, ns.ccnSIM.ccn.Data):
                self._data = data
            else:
    ...
    @staticmethod
    def fromWire (wire):
        return Data (data = ns.ccnSIM.ccn.Wire.ToDataStr (bytes (wire)))

    def sign (self, key):
        """There is no actual signing in ccnSIM for now, but we will assign signature bits based on key"""
    ...
    def toWire (self):
        ...
        return ns.ccnSIM.ccn.Wire.FromDataStr (self._data)

    def verify_signature (self, key):
        """There is no actual signing in ccnSIM for now, but we will check if signature matches the key"""
        return True
    ...
    def __getattr__ (self, name):
        if name == "_data":
            ...
            return ns.ccnSIM.ccn.PacketToBuffer (pkt)
        else:
    ...
    def __setattr__(self, name, value):
        if name == "_data":
            ...
            if value is None:
                return self._data.SetName (ns.ccnSIM.ccn.Name ())
            elif isinstance (value, Name):
                return self._data.SetName (value._name)
            elif isinstance (value, ns.ccnSIM.ccn.Name):
                return self._data.SetName (value)
            elif isinstance (value, str):
                return self._data.SetName (ns.ccnSIM.ccn.Name (value))
            else:
            ...
            if value is None:
                ...
                pkt = ns.ccnSIM.ccn.BufferToPacket (bytes (value))
                self._data.SetPayload (pkt)
        else:
            raise NameError ("Unknown attribute [%s]" % name)

    def __repr__(self):
        return "ccn.Data(%s; %s)" % str (self._data, self.signedInfo)
