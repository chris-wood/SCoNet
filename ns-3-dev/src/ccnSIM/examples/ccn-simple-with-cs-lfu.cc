 ...
 *
 *     NS_LOG=ccn.Consumer ./waf --run ccn-simple-with-cs-lfu
 */
...
main (int argc, char *argv[])
{
  ...
  ccnxHelper.SetContentStore ("Nocache"); // disable cache
  ccnxHelper.Install (nodes.Get (0));

  // node 1 and 2: set cache with Lfu policy
  ccnxHelper.SetContentStore ("Lru", "MaxSize", "2"); // can set cache size this way
  ...
  consumerHelper.SetAttribute ("Frequency", StringValue ("5")); // 10 interests a second
  ...
  Simulator::Schedule (Seconds (5), PrintCsMemStats, boost::ref (std::cout), Seconds (100), beginRealTime);
