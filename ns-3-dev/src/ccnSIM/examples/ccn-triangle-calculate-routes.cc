 ...
 */
// ccn-triangle-calculate-routes.cc
...
main (int argc, char *argv[])
{
  ...
  ccnGlobalRoutingHelper.AddOrigins ("/test/prefix", Names::Find<Node> ("C1"));
  ccnGlobalRoutingHelper.AddOrigins ("/test/prefix", Names::Find<Node> ("C2"));
  ccn::GlobalRoutingHelper::CalculateRoutes ();

  cout << "FIB content on node A1" << endl;
  Ptr<ccn::Fib> fib = Names::Find<Node> ("A1")->GetObject<ccn::Fib> ();
  for (Ptr<ccn::fib::Entry> entry = fib->Begin (); entry != fib->End (); entry = fib->Next (entry))
    {
      cout << *entry << " (this is towards: ";
      cout << Names::FindName (DynamicCast<const ccn::NetDeviceFace> (entry->FindBestCandidate (0).GetFace ())->GetNetDevice ()->GetChannel ()->GetDevice (1)->GetNode ());
      cout << ")" << endl;
    }

  cout << "FIB content on node A2" << endl;
  fib = Names::Find<Node> ("A2")->GetObject<ccn::Fib> ();
  for (Ptr<ccn::fib::Entry> entry = fib->Begin (); entry != fib->End (); entry = fib->Next (entry))
    {
      cout << *entry << " (this is towards: ";
      cout << Names::FindName (DynamicCast<const ccn::NetDeviceFace> (entry->FindBestCandidate (0).GetFace ())->GetNetDevice ()->GetChannel ()->GetDevice (1)->GetNode ());
