...
#ifndef CCN_IP_FACES_HELPER_H
#define CCN_IP_FACES_HELPER_H
...
/**
 * @ingroup ccn-helpers
 * @brief Helper for CCN IP-based face creation
 */
class IpFacesHelper
{
public:
   ...
   * to CCN stack and add the requested route
   *
   * If face has been already created before (same IP address), then this call will simply
   * update FIB with requested prefix
   */
  static void
  CreateTcpFace (const Time &when, Ptr<Node> node, Ipv4Address address, const std::string &prefix, int16_t metric = 1, int32_t rank = 1);

  /**
   * @brief Create TCP face
   */
  static void
  CreateUdpFace (const Time &when, Ptr<Node> node, Ipv4Address address, const std::string &prefix, int16_t metric = 1, int32_t rank = 1);
};

} // namespace ccn
} // namespace ns3

#endif // CCN_IP_FACES_HELPER_H
