...
NS_LOG_COMPONENT_DEFINE ("ccn.TcpFace");
...
public:
  ...
  {
    static TypeId tid = TypeId ("ns3::ccn::TcpFace::BoundaryHeader")
      .SetGroupName ("Ccn")
      .SetParent<Header> ()
      ;
    return tid;
...
TcpFace::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::ccn::TcpFace")
    .SetParent<Face> ()
    .SetGroupName ("Ccn")
    ;
  return tid;
...
void
TcpFace::RegisterProtocolHandlers (const InterestHandler &interestHandler, const DataHandler &dataHandler, const InterestResponseHandler &interestResponseHandler)
{
...
TcpFace::OnConnect (Ptr<Socket> socket)
{
  ...
  Ptr<L3Protocol> ccn = GetNode ()->GetObject<L3Protocol> ();
