  ...
  BOOST_FOREACH (const Link &link, m_linksList)
    {
...

      {
        ...
        Ptr<ccn::L3Protocol> ccn = link.GetFromNode ()->GetObject<ccn::L3Protocol> ();
        if (ccn != 0)
          {
            Ptr<ccn::Face> face = ccn->GetFaceByNetDevice (link.GetFromNetDevice ());
            NS_ASSERT (face != 0);
...

      {
        ...
        Ptr<ccn::L3Protocol> ccn = link.GetToNode ()->GetObject<ccn::L3Protocol> ();
        if (ccn != 0)
          {
            Ptr<ccn::Face> face = ccn->GetFaceByNetDevice (link.GetToNetDevice ());
            NS_ASSERT (face != 0);
