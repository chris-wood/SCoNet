...
#ifndef CCN_CONSUMER_H
#define CCN_CONSUMER_H
...
#include "ns3/ccn-rtt-estimator.h"
...
/**
 * @ingroup ccn-apps
 * \brief CCN application for sending out Interest packets
 */
...
protected:
  ...
  /**
   * \brief Constructs the Interest packet and sends it using a callback to the underlying CCN protocol
   */
  virtual void
  ...
  */
  virtual bool
  NameKeyIdCheck(Ptr<const Packet> dataKeyId);
  ...
  */
  virtual bool
  NameHashCheck(Ptr<const Packet> dataHash);
  /**
  * \brief Check the Name Nonce  
  * 
  */
  virtual bool
  NameNonceCheck(uint32_t dataNonce);
protected:
  ...
  Name            m_interestName;        ///< \brief CCN Name of the Interest (use Name)
...
/// @cond include_hidden
  ...
  TracedCallback<Ptr<App> /* app */, uint32_t /* seqno */,
                 Time /* delay */> m_lastRetransmittedInterestDataDelay;
  TracedCallback<Ptr<App> /* app */, uint32_t /* seqno */,
                 Time /* delay */, uint32_t /*retx count*/> m_firstInterestDataDelay;
