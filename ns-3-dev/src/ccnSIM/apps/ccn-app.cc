...
NS_LOG_COMPONENT_DEFINE ("ccn.App");
...
App::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ccn::App")
    .SetGroupName ("Ccn")
    ...
    .AddTraceSource ("ReceivedInterestResponses", "ReceivedInterestResponses",
                    MakeTraceSourceAccessor (&App::m_receivedInterestResponses))
    ...
    .AddTraceSource ("TransmittedInterestResponses", "TransmittedInterestResponses",
                    MakeTraceSourceAccessor (&App::m_transmittedInterestResponses))
...
App::DoDispose (void)
{
  ...
  // The best reason I see is that apps are freed after ccn stack is removed
  // StopApplication ();  
...
void
App::OnInterestResponse (Ptr<const InterestResponse> interestResponse)
{
  NS_LOG_FUNCTION (this << interestResponse);
  m_receivedInterestResponses (interestResponse, this, m_face);
}
...
App::StartApplication () // Called at time specified by Start
{
  ...
  NS_ASSERT_MSG (GetNode ()->GetObject<L3Protocol> () != 0,
                 "Ccn stack should be installed on the node " << GetNode ());

  // step 1. Create a face
  m_face = CreateObject<AppFace> (/*Ptr<App> (this)*/this);
    
  // step 2. Add face to the Ccn stack
...
App::StopApplication () // Called at time specified by Stop
{
  ...
  // step 2. Remove face from Ccn stack
  GetNode ()->GetObject<L3Protocol> ()->RemoveFace (m_face);

  // step 3. Destroy face
  if (m_face->GetReferenceCount () != 1)
    {
      NS_LOG_ERROR ("Please a bug report on https://github.com/CCN-Routing/ccnSIM/issues");
