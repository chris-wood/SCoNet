...
To obtain simulation results, you would need to connect to one or more `trace sources <doxygen/group___trace_source_list.html>`_ provided by ccnSIM classes.
...
- :ccnsim:`ccn::L3AggregateTracer`

    Tracing the aggregate number of Interests/Data packets forwarded by an CCN node
    ...
    .. code-block:: c++
        ...
        ccn::L3AggregateTracer::InstallAll ("aggregate-trace.txt", Seconds (1.0));
...
- :ccnsim:`ccn::L3RateTracer`

    Tracing the rate in bytes and in number of packets of Interest/Data packets forwarded by an CCN node
    ...
    .. code-block:: c++
        ...
        ccn::L3RateTracer::InstallAll ("rate-trace.txt", Seconds (1.0));
...
- :ccnsim:`L2Tracer`
    ...
    This tracer is similar in spirit to :ccnsim:`ccn::L3RateTracer`, but it currently traces only packet drop on layer 2 (e.g.,
...
This example (``ccn-tree-tracers.cc``) demonstrates basic usage of :ref:`trace classes`.
...
Example simulation (``ccn-tree-tracers.cc``) scenario that utilizes trace helpers:

.. literalinclude:: ../../examples/ccn-tree-tracers.cc
    :language: c++
    :linenos:
    :lines: 21-28,61-
...
This example (``ccn-tree-with-l2tracer.cc``) demonstrates basic usage of :ref:`trace classes`.
...
Example simulation (``ccn-tree-with-l2tracer.cc``) scenario that utilizes trace helpers:

.. literalinclude:: ../../examples/ccn-tree-with-l2tracer.cc
    :language: c++
    :linenos:
    :lines: 1-
...
To run this scenario, use the following command::
        ...
        ./waf --run=ccn-tree-with-l2tracer
...
- :ccnsim:`ccn::CsTracer`
    ...
    With the use of :ccnsim:`ccn::CsTracer` it is possible to obtain statistics of cache hits/cache misses on simulation nodes.
    ...
    .. code-block:: c++
        ...
        ccn::CsTracer::InstallAll ("cs-trace.txt", Seconds (1));
...
This example (``ccn-tree-cs-tracers.cc``) demonstrates basic usage of content store tracer.
...
Example simulation (``ccn-tree-cs-tracers.cc``) scenario that utilizes trace helpers:

.. literalinclude:: ../../examples/ccn-tree-cs-tracers.cc
    :language: c++
    :linenos:
    :lines: 21-28,61-
    :emphasize-lines: 59
...
- :ccnsim:`ccn::AppDelayTracer`

    With the use of :ccnsim:`ccn::AppDelayTracer` it is possible to obtain data about for delays between issuing Interest and receiving corresponding Data packet.
    ...
    |                 | an AppFace (but not from application to ccn::L3Protocol!).          |
    |                 |                                                                     |
...
This example (``ccn-tree-app-delay-tracer.cc``) demonstrates basic usage of application-level Interest-Data delay tracer.
...
Example simulation (``ccn-tree-app-delay-tracer.cc``) scenario that utilizes trace helpers:

.. literalinclude:: ../../examples/ccn-tree-app-delay-tracer.cc
    :language: c++
    :linenos:
    :lines: 21-28,61-
    :emphasize-lines: 59
...
This example (``ccn-simple-with-pit-count-stats.cc``) shows how you can periodically print out current size of PIT on the selected nodes.

.. literalinclude:: ../../examples/ccn-simple-with-pit-count-stats.cc
    :language: c++
    :linenos:
    :lines: 20-26,47-
...
To run this scenario, use the following command::
        ...
        ./waf --run=ccn-simple-with-pit-count-stats
