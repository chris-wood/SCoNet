...
#include "ccn-l3-aggregate-tracer.h"
...
NS_LOG_COMPONENT_DEFINE ("ccn.L3AggregateTracer");
...
L3AggregateTracer::OutNacks  (Ptr<const Interest> nack, Ptr<const Face> face)
{
  ...
  if (nack->GetTlv ())
    {
      m_stats[face].get<1> ().m_outNacks += (nack->GetTlv ())->GetSerializedSize ();
    }
}
...
L3AggregateTracer::InNacks   (Ptr<const Interest> nack, Ptr<const Face> face)
{
  ...
  if (nack->GetTlv ())
    {
      m_stats[face].get<1> ().m_inNacks += (nack->GetTlv ())->GetSerializedSize ();
    }
}
...
L3AggregateTracer::DropNacks (Ptr<const Interest> nack, Ptr<const Face> face)
{
  ...
  if (nack->GetTlv ())
    {
      m_stats[face].get<1> ().m_dropNacks += (nack->GetTlv ())->GetSerializedSize ();
    }
}
